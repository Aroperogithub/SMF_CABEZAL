...
int protocolo_Test_SN_RECEIVER (char *lora_sn_receiver) {
  ...
  receiver_local[i] = '\0';
  Serial.print (F("SN RECEIVER LEIDO: "));
  protocolo_PRINTLN_STRING (receiver_local);
  if (!strcmp(receiver_local, lora_sn_receiver)) {  
    Serial.println (F("RECEIVER == SN-RELES"));
...
int protocolo_Extraer_PAYLOAD (String pakete, char *mensaje, int lenght_payload, int total_length) {
  ...
  if (strstr (mensaje, "#VINCULAR*") != NULL) {
    ...
    return LORA_ACK;
  } else if (strstr (mensaje, "#TESTNIVELES*") != NULL) {
    return LORA_TEST_NIVELES;
  } else if (strstr (mensaje, "#VINC_FORZADA*") != NULL) {
    return LORA_VINCULACION_FORZADA;
  } else {
...
float protocolo_Extraer_INT (char *token) {
  ...
  if (nivel != NULL) {
    ...
    for (int i = 7; i < strlen(nivel); i++) {
      nivel_str[i - 7] = nivel[i];
    }
  ...
  if (media != NULL) {
    ...
    for (int i = 7; i < strlen(media); i++) {
      media_str[i - 7] = media[i];
    }
  ...
  if (desv_tip != NULL) {
    ...
    for (int i = 5; i < strlen(desv_tip); i++) {
      desv_tip_str[i - 5] = desv_tip[i];
    }
...
int protocolo_Mensaje_pruebas (char *mensaje_, int long_mensaje, char *mensaje_local) {
  int indice = 0;
  char long_local[6];
  ...
  if (strlen (sn_receptioner) == 0) {
    strcpy (sn_receptioner, sn_receiver);
  }
  for (int i2 = 0; i2 < DATOS_NUMERO_SERIE; i2++) {
    mensaje_local[indice++] = sn_receptioner[i2];
  }
